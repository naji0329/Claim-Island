...
const ConnectPool = (props) => {
  ...
  const purchaseShell = async (connectedAccount) => {
    ...
    try {
      await buyShellPresale(
        { connectedAccount, amount: bnbAmount },
        props.callback,
        props.errCallback
      );
    } catch (e) {
  ...
  const disableButton =
    ...
    INDIVIDUAL_CAP === Number(individualLimit);
                ...
                >
                  <ModalHeader toggle={toggle}>
                    ...
                    <div>
                      ...
                      <Badge color="dark" pill={true}>
                        You can use up to{" "}
                        {(INDIVIDUAL_CAP - Number(individualLimit)).toFixed(2)}{" "}
                        BNB to purchase $SHELL
                      </Badge>
                    </div>
                  </ModalHeader>
                  <ModalBody>
                        ...
                        {purchaseAmount > 0 && purchaseAmount <= 3 && (
                          <p>
                            ...
                            {Number(purchaseAmount) *
                              Number(presaleRate.toString())}{" "}
                            in SHELL
                          </p>
                        )}
                        {Number(purchaseAmount) + Number(individualLimit) >
                          INDIVIDUAL_CAP && (
